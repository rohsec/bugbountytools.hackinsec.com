...
for i in x1:
            if(i.startswith("#")):
                ...
                display(HTML(f"<a target='_blank' href='https://google.com/search?q={i[1:]}'>{i[1:]}</a>"))
    ...
    if(re.match(domain_pattern,inp.value)):
        ...
        for i in x1:
            if(i.startswith("#")):
                ...
                display(HTML(f"<a target='_blank' href='https://google.com/search?q={i.replace('example.com',inp.value)[1:]}'>{i.replace('example.com',inp.value)[1:]}</a>"))
    # print(x1)
    else:
        div.innerHTML="<span>Please insert a valid domain !!</span>"
